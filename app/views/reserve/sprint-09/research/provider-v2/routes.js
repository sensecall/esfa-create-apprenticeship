...
// Bulk single option
router.post('/funding--bulk-or-single', (req, res) => {
	if (req.session.data['bulk-single'] == 'no' ) {
		res.redirect(`funding--choose-employer`)
	} else {
		res.redirect(`funding--bulk-enter-details`)
	}
})

function monthsRadios(){
...
router.get('/funding--bulk-enter-details', (req, res) => {
	var months = monthsRadios()

	res.render(`${req.feature}/funding--bulk-enter-details`,{months})
})
